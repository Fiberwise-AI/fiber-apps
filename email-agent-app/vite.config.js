...
let appId = 'email-agent-app'; // Default or placeholder
...
export default defineConfig({
  ...
  build: {
    ...
    //outDir: path.resolve(__dirname, `../../app_bundles/apps/${appId}/${versionId}/dist`),

    emptyOutDir: true, // Clean output directory before build
    minify: false, // Disable minification to keep function names readable
    rollupOptions: {
      input: path.resolve(__dirname, 'index.js'), // Single entry point
      output: {
      ...
      // Don't mark any imports as external - we want everything bundled
      external: [], 
    },
    // commonjsOptions: {
    //   include: [/node_modules/] // Include SDK files in the bundle
    // },
    // Include assets like HTML/CSS if imported directly
    assetsInclude: ['**/*.html?raw', '**/*.css?raw'],
    sourcemap: true, // Enable sourcemaps for easier debugging
    // Configure as a library build to preserve exports
    lib: {
  ...
  server: {
    ...
    port: 5558, // Different port from base-app
  },
  plugins: [
    // Add custom plugins if needed, e.g., for raw HTML/CSS imports
    // Example for handling "?raw" imports (Vite >= 2.9)
    // No specific plugin needed for "?raw" imports in modern Vite versions
  ],
  resolve: {
