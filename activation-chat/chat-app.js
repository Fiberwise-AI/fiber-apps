...
export class ActivationChatApp extends HTMLElement {
  ...
  async loadAgents() {
    try {
      ...
      console.log('[ActivationChat] Agents response:', response);
      
      // Handle different response structures
      const agents = response.agents || response.items || response || [];
      console.log('[ActivationChat] Parsed agents array:', agents);
      
      this.selectedAgentId = agents.length > 0 ? (agents[0].agent_id || agents[0].id) : null;
      console.log('[ActivationChat] Selected agent ID:', this.selectedAgentId);
  ...
  async sendMessage(content) {
    ...
    if (!this.selectedAgentId) {
      console.error('[ActivationChat] No agent selected - cannot send message');
      console.log('[ActivationChat] Available providers:', this.providers);
      console.log('[ActivationChat] Attempting to reload agents...');
      await this.loadAgents();
      if (!this.selectedAgentId) {
    ...
    try {
      ...
      const metadata = {};
      console.log('[ActivationChat] Building metadata, selectedProviderId:', this.selectedProviderId);
      console.log('[ActivationChat] Available providers:', this.providers);
      if (this.selectedProviderId) {
        ...
        if (provider) {
          ...
          console.log('[ActivationChat] Provider found, metadata after adding provider:', metadata);
          console.log('[ActivationChat] Metadata keys:', Object.keys(metadata));
          console.log('[ActivationChat] Metadata entries:', Object.entries(metadata));
        } else {
          console.log('[ActivationChat] Provider not found in providers list');
        }
      } else {
        console.log('[ActivationChat] No selectedProviderId set');
      }

      console.log('[ActivationChat] About to activate agent with:');
      console.log('[ActivationChat] - selectedAgentId:', this.selectedAgentId);
      console.log('[ActivationChat] - input:', { prompt: content });
      console.log('[ActivationChat] - context:', { chat_id: this.currentChatId, system_prompt: this.modelSettings.systemPrompt });
      console.log('[ActivationChat] - metadata:', metadata);
      console.log('[ActivationChat] - metadata stringified:', JSON.stringify(metadata, null, 2));

      // Try adding provider_id to context as well
      const context = {
      ...
      console.log('[ActivationChat] - final context:', context);
      ...
      console.log('[ActivationChat] Activation result:', activation);
  ...
  render() {
      ...
      if (chatMessages) {
        chatMessages.innerHTML = `
          <div class="empty-state">
            ...
            <button class="primary-button provider-link" onclick="this.getRootNode().host.navigateToSettings()">
    ...
    if (chatMessages) {
      ...
      console.log('[ActivationChat] About to set selected provider:', this.selectedProviderId);
      if (this.selectedProviderId) {
        chatMessages.setSelectedProvider(this.selectedProviderId);
      } else {
        console.log('[ActivationChat] No selectedProviderId to set');
      }
      ...
      chatMessages.addEventListener('provider-selected', (e) => {
        ...
        console.log('Provider selected from event:', this.selectedProviderId);
