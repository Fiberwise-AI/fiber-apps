...
class ChatAgent(FiberAgent):
    ...
    async def run_agent(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(f"ChatAgent processing message in chat {chat_id}")
            ...
            conversation_context = await self._get_conversation_context(fiber, chat_id)
            ...
            if llm_service:
                ...
                return "I apologize, but I don't have access to an LLM service to generate a response. Please check the system configuration."
    ...
    async def _get_conversation_context(self, fiber, chat_id: str) -> Optional[str]:
        """
...
Please continue the conversation naturally, building on what has been discussed while addressing the user's current message."""
    ...
    async def _generate_llm_response(
        ...
        try:
            ...
            # Generate response - provider_id is already in the LLM service from initialization
            ...
            if response and isinstance(response, dict):
                # Handle new LLM service response format with status field
                response_text = None
                if "text" in response:
                    response_text = response["text"].strip()
                elif response.get("status") == "completed" and "text" in response:
                    response_text = response["text"].strip()
                ...
                if response_text:
                    ...
                    logger.warning(f"LLM service response missing text field: {response}")
                    return "I apologize, but I received an incomplete response from the language model. Please try your request again."
            else:
                logger.warning(f"LLM service returned unexpected response format: {response} (type: {type(response)})")
                return "I apologize, but I received an unexpected response format from the language model. Please try your request again."
